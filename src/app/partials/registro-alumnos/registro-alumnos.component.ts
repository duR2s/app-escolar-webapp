...
export class RegistroAlumnosComponent implements OnInit, OnChanges { // Implementamos OnChanges
  ...
  ngOnInit(): void {
    ...
    if (this.activatedRoute.snapshot.params['id'] != undefined) {
      ...
      //Al iniciar la vista asignamos los datos del user (puede estar vacío al inicio)
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['datos_user'] && changes['datos_user'].currentValue) {
      if (this.editar && Object.keys(changes['datos_user'].currentValue).length > 0) {
        ...
        console.log("Datos actualizados dinámicamente en Alumno (ngOnChanges):", this.alumno);
      }
    }
  }
