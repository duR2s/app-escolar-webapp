...
export class RegistroAdminComponent implements OnInit, OnChanges { // Implementamos OnChanges
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe(params => {
      if (params['id']) {
        ...
        // Al iniciar la vista asignamos los datos del user, aunque pueden venir vacíos inicialmente
        // ngOnChanges se encargará de actualizar esto cuando la API responda.
      ...
      console.log("Admin OnInit: ", this.admin);
    });
  }

  // MÉTODO AGREGADO: Detecta cambios en las propiedades @Input
  ngOnChanges(changes: SimpleChanges): void {
    // Verificamos si la propiedad 'datos_user' ha cambiado
    if (changes['datos_user'] && changes['datos_user'].currentValue) {
      // Si estamos en modo edición o si el objeto que llega no está vacío
      if (this.editar && Object.keys(changes['datos_user'].currentValue).length > 0) {
        this.admin = changes['datos_user'].currentValue;
        console.log("Datos actualizados dinámicamente en Admin (ngOnChanges):", this.admin);
      }
    }
  }
